...
import { getDayOfNextWeekTimestamp } from '../dates/get-day-of-next-week-timestamp';
...
export const formatEnrollmentMessage = (client: Client) => {
  const enrollmentMessageContent =
    `**- Ouverture des inscriptions pour la semaine du ${formatDiscordTimestamp(
      getDayOfNextWeekTimestamp(1),
      {
        shorten: true,
      },
    )} au ${formatDiscordTimestamp(getDayOfNextWeekTimestamp(7), {
      shorten: true,
    })} -** \n\n` +
